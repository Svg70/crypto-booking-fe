...
export const toWad = (amount: string, decimals = 18): BigNumberish => {
  return ethers.parseUnits(sanitizeDecimals(amount, decimals), decimals);
};

export const fromWad = (wad: BigNumberish, decimals = 18): string => {
  return sanitizeDecimals(ethers.formatUnits(wad, decimals), decimals);
};
...
export const getProviderUrl = (chainId: string) => {
  return `https://rpc.walletconnect.com/v1/?chainId=${chainId}&projectId=${process.env.REACT_APP_WALLET_CONNECT_PROJECT_ID}`;
};
